...
// System prompts
const MERCHANT_SYSTEM_PROMPT = `あなたはAIden（アイデン）のカスタマーサポートAIアシスタントです。
AIdenは日本の飲食店向けオールインワンSaaSプラットフォームで、ブランドHP、モバイルオーダー、注文ダッシュボード、顧客管理、管理マスタを提供しています。
...
const ENDUSER_SYSTEM_PROMPT = `あなたはAIden（アイデン）のカスタマーサポートAIアシスタントです。
...
- 解決できない場合は「support@aiden-jp.net にお問い合わせください」と案内してください
...
async function handleSend(req, res) {
  ...
  try {
    ...
    aiResponse = isMerchant
      ? '申し訳ございません。現在AIアシスタントに接続できません。support@aiden-jp.net までお問い合わせください。'
      : '現在チャットが利用できません。support@aiden-jp.net までお問い合わせください。';
  }
