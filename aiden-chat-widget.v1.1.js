...
class AidenChatWidget {
  ...
  _injectStyles() {
    ...
    style.textContent = `
      ...
      .acw-window.acw-minimized {
        display: flex;
        height: auto;
        min-height: auto;
        max-height: none;
      }
      .acw-window.acw-minimized .acw-messages,
      .acw-window.acw-minimized .acw-input-area {
        display: none;
      }
  ...
  _createDOM() {
    ...
    this.windowEl.innerHTML = `
      <div class="acw-header">
        ...
        <div class="acw-header-actions">
          <button class="acw-minimize" aria-label="最小化" title="最小化">&#8722;</button>
    ...
    this.minimizeBtn = this.windowEl.querySelector('.acw-minimize');

    // Apply minimized state from localStorage
    if (this.isMinimized) {
      this.windowEl.classList.add('acw-open', 'acw-minimized');
  ...
  _attachEvents() {
    ...
    // Minimize/Maximize
    this.minimizeBtn.addEventListener('click', () => this._toggleMinimize());
    ...
    // Keyboard shortcuts: ESC to close, Shift+? to toggle
    document.addEventListener('keydown', (e) => {
      // Skip if user is typing in an input/textarea (except our own)
      const tag = (e.target.tagName || '').toLowerCase();
      const isOurInput = e.target === this.inputEl;
      if ((tag === 'input' || tag === 'textarea' || e.target.isContentEditable) && !isOurInput) return;

      if (e.key === 'Escape' && this.isOpen) {
        this.close();
      } else if (e.key === '?' && e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey) {
        // Shift+? to toggle (only when not typing in other inputs)
        if (!isOurInput) {
          e.preventDefault();
          this.toggle();
        }
  ...
  open() {
    ...
    this.windowEl.classList.remove('acw-minimized');
    this.btnEl.style.display = 'none';
    this.minimizeBtn.innerHTML = '&#8722;';
    this.minimizeBtn.title = '最小化';
    localStorage.setItem(this.minimizeKey, 'false');
    this.inputEl.focus();
  ...
  close() {
    this.isOpen = false;
    this.isMinimized = false;
    this.windowEl.classList.remove('acw-open', 'acw-minimized');
  ...
  _toggleMinimize() {
    if (this.isMinimized) {
      ...
      this.windowEl.classList.add('acw-minimized');
      this.minimizeBtn.innerHTML = '&#9723;';
      this.minimizeBtn.title = '最大化';
      localStorage.setItem(this.minimizeKey, 'true');
    }
  }
  ...
  async _ensureSession() {
    ...
    if (this.storeId) row.store_id = this.storeId;
    if (this.brandId) row.brand_id = this.brandId;
    if (this.memberId) row.customer_id = this.memberId;
    if (this.operatorId) row.operator_id = this.operatorId;
    if (this.guestSessionId) row.guest_session_id = this.guestSessionId;

    const { data, error } = await this.sb.from('chat_sessions').insert(row).select('id').single();
  ...
  async _saveMessageToDB(role, content) {
    if (!this.sb || !this.sessionId) return null;
    const { data, error } = await this.sb.from('chat_messages').insert({
  ...
  async _sendMessage() {
    ...
    try {
      ...
      await this._saveMessageToDB('user', text);
      ...
      try {
        ...
        if (resp.ok) {
          ...
          aiReply = data.message ? data.message.content : null;
      ...
      // Fallback: fixed response when API/EF is not deployed
      if (!aiReply) {
        aiReply = 'AIサポートは準備中です。お問い合わせはsupport@aiden-jp.netまでメールでお願いします。';
      }

      // Save AI reply to DB
      const msgId = await this._saveMessageToDB('assistant', aiReply);

      // Show assistant message
      this._appendMessage('assistant', aiReply, msgId);
  ...
  async _sendFeedback(messageId, feedbackVal, btnUp, btnDown) {
    try {
      // Try Supabase direct update first
      if (this.sb) {
        await this.sb.from('chat_messages').update({ feedback: feedbackVal }).eq('id', messageId);
      } else {
        await fetch(`${this.apiBase}/api/chat/feedback`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message_id: messageId, feedback: feedbackVal }),
        });
      }
  ...
  async _loadHistory() {
    ...
    try {
      ...
      if (this.sb) {
        ...
        // Session may have been cleaned up — reset
        if (error || (messages && messages.length === 0)) {
          // Verify session still exists
          const { data: sess } = await this.sb
            .from('chat_sessions')
            .select('id')
