...
**Goal:** aiden-admin.html をオーバーホール — KPIダッシュボード/CS管理ページ削除、ダッシュボードDB連動化、法人管理DB永続化、操作ログページ追加、手数料設定UI追加

**Architecture:** 単一ファイル `aiden-admin.html` の大規模改修。既存のSupabase Client（`sb`変数）を使ったDB連携パターンを踏襲。KPI/CSの~480行を削除し、操作ログ/手数料設定の新機能を追加。ダッシュボードのハードコードデータをaudit_logs/ordersテーブルからのDB取得に置換。

**Tech Stack:** HTML/Vanilla JS, Supabase JS Client v2, Chart.js

**File:** `aiden-admin.html` (3532行 → 修正後 ~3300行)
...
**Files:**
- Modify: `aiden-admin.html:96-98` (sidebar menu items)
- Modify: `aiden-admin.html:385` (updateSidebar mapping)
- Modify: `aiden-admin.html:417-418` (renderPage routing)
- Delete: `aiden-admin.html:2939-3159` (KPI dashboard code ~220 lines)
- Delete: `aiden-admin.html:3161-3424` (CS management code ~264 lines)
- Modify: `aiden-admin.html:3520-3530` (chat widget + CS hash navigation)
...
**Files:**
- Modify: `aiden-admin.html` sidebar section (~L94-99)
- Modify: `aiden-admin.html` updateSidebar mapping
- Modify: `aiden-admin.html` renderPage routing
...
現状のaiden-admin.htmlにはフッターHTML・サポートボタンHTML自体が存在しない（チャットウィジェットはTask 1で削除済み）。追加の作業不要。
...
**Files:**
- Modify: `aiden-admin.html` — 新しいセクション追加（CS管理削除跡に配置）
...
**Files:**
- Modify: `aiden-admin.html` — `renderDashboard()` 関数 (L432-478) を完全書き換え
- Modify: `aiden-admin.html` — `renderDashCharts()` 関数 (L479) を書き換え
- Modify: `aiden-admin.html` — `renderPage()` 関数にダッシュボード初期化フック追加
...
**Files:**
- Modify: `aiden-admin.html` — `loadAllData()` にaccountsテーブル読み込み追加
- Modify: `aiden-admin.html` — `saveAccount()`, `saveEditAccount()`, `deleteAccount()` をDB永続化
...
**Files:**
- Modify: `aiden-admin.html` — `renderCorpPage()` (L1231-1243)
...
**Files:**
- Modify: `aiden-admin.html` — `renderCorpPage()` にタブ追加
...
**Files:**
- Modify: `aiden-admin.html` — `renderSystem()` (L709-715)
