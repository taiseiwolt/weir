# weir-admin.html「注文管理」セクション実装計画（ULTRAPLAN）

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** weir-admin.html にプラットフォーム全体の注文を横断的に閲覧・操作できる「注文管理」ページを新規追加する。
...
### 5. Realtime パターン（weir-order-dashboard.html 1200〜1265 行参照）
```js
...
1. **ゲストPIIの表示方針**
   CLAUDE.md の「ゲストPII非共有」ルールは「事業者に見せない」意味で、platform_admin（Weir社内）は見てよい前提のはず。依頼書には `customer_name / customer_phone / customer_email` を詳細モーダルに表示すると明記されている。
...
- Modify: `/Users/taisei/Desktop/aiden-demo/weir-admin.html`（唯一の変更対象）
...
**Files:**
- Modify: `weir-admin.html:91`（新規 nav-item 追加）
- Modify: `weir-admin.html:429`（sidebarPage マップ）
- Modify: `weir-admin.html:454-477`（renderPage 分岐）
...
**Files:**
- Modify: `weir-admin.html` JS セクション末尾付近（renderAuditLogPage のすぐ後 or 独立セクションとして追記）
...
**Files:**
- Modify: `weir-admin.html`（renderOrdersPage の後に追記）
...
**Files:**
- Modify: `weir-admin.html`（前タスクの後に追記）
...
- 別タブで新規注文を作成（weir-order-store.html から） → 管理マスタの一覧に自動反映
...
- 既存関数（`escHtml`, `showToast`, `openModal`, `closeModal`, `logAudit`, `getBrandsForMerchant`, `getVenuesForBrand`, `getVenuesForMerchant`, `sb`, `MERCHANTS`, `BRANDS`, `VENUES`, `currentView`）は weir-admin.html に既存
...
- [ ] 本番URL(weir.co.jp/weir-admin.html)での動作確認
