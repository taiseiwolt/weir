...
**Architecture:** 各HTMLファイルの個別修正。共通ファイル（aiden-common.js/css）への変更はサインインボタンのonclick追加のみ。sessionStorageを使ったページ間データ受け渡し改善。
...
| Action | File | Changes |
|--------|------|---------|
| Modify | `aiden-order.html` | マップピンブランドカラー化、住所データsessionStorage保存、確認テキスト1行化 |
| Modify | `aiden-order-store.html` | タイトルホワイトラベル化、beforeunload削除、初期通知非表示、サインイン接続、住所引継ぎ |
| Modify | `aiden-order-checkout.html` | 住所プリフィル、クーポンUI改善、チャットウィジェット追加 |
| Modify | `aiden-common.js` | サインインボタンにonclick属性追加（各ページのコールバック呼出し） |

---

## Task 1: aiden-order.html — マップピンをブランドカラー化

**Files:**
- Modify: `aiden-order.html:1184-1205`

- [ ] **Step 1: makeStoreIcon()のハードコードカラーをCSS変数から取得に変更**

`aiden-order.html` line 1184-1205 の `makeStoreIcon` 関数を修正:
...
## Task 2: aiden-order.html — 住所データのsessionStorage保存 + 確認テキスト1行化

**Files:**
- Modify: `aiden-order.html:675,975,994,1639-1653,1750-1762`
...
## Task 3: aiden-order-store.html — タイトルホワイトラベル化 + beforeunload削除
...
## Task 4: aiden-order-store.html — 初期通知非表示 + 住所引き継ぎ

**Files:**
- Modify: `aiden-order-store.html:727,1014-1020`
...
## Task 5: aiden-order-store.html — サインインボタン接続

**Files:**
- Modify: `aiden-common.js:424`
- Modify: `aiden-order-store.html` (onBrandLoaded callback付近)
- Modify: `aiden-order-checkout.html` (signinモーダル関連)

- [ ] **Step 1: aiden-common.jsのサインインボタンにid追加**

Line 424 で既にボタンは生成されているが、idがない。`id="aiden-header-signin"` を追加:

aiden-common.js line 424:
```javascript
...
var signinBtn = document.getElementById('aiden-header-signin');
if (signinBtn) {
...
```bash
git add aiden-common.js aiden-order-store.html
git commit -m "fix: add id to header signin button and connect click handler"
```

---

## Task 6: aiden-order-store.html — ハードコードフォールバック汎用化

**Files:**
- Modify: `aiden-order-store.html:945-960`
...
## Task 7: aiden-order-checkout.html — 住所プリフィル

**Files:**
- Modify: `aiden-order-checkout.html` (initPage / loadCheckoutData 付近)
...
## Task 8: aiden-order-checkout.html — クーポンUI改善

**Files:**
- Modify: `aiden-order-checkout.html:646-667`
...
## Task 9: aiden-order-checkout.html — チャットウィジェット追加

**Files:**
- Modify: `aiden-order-checkout.html` (`</body>` 直前)

- [ ] **Step 1: aiden-chat-widget.jsの読込を追加**

`</body>` 直前（`<div id="aiden-footer"></div>` の後）に追加:
