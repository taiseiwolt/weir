...
#### P0-2: 管理画面（aiden-admin.html）にロールベースアクセス制御なし
- **対象**: `aiden-admin.html` lines 237-240
- **内容**: 認証チェックが `if(!session){window.location.href='/';return;}` のみで、ユーザーが管理者かどうかの検証なし。チェックアウトページで登録したエンドユーザーでも管理画面にアクセス可能。
- **再現手順**: エンドユーザーとしてサインアップ → `aiden-admin.html`に直接アクセス
...
- **対象**:
  ...
  - `aiden-order-checkout.html` line 1208: `item.img`がsessionStorageから取得（DB由来）、`" onerror="alert(1)`で攻撃可能
...
- **対象**: プロジェクトルート
- **内容**: 検索エンジンが`aiden-admin.html`、`aiden-customer-admin.html`、`aiden-order-dashboard.html`をインデックス可能
...
- **対象**: `aiden-order-dashboard.html` line 641, `aiden-store.html` line 1036, `aiden-order-tracking.html` line 251
...
- **対象**: 30ファイル（aiden-customer-admin.html: 38件、aiden-order-checkout.html: 14件等）
- **影響**: ブラウザコンソール経由の内部情報漏洩
- **修正方針**: `npm run lint`で検出・削除
- **フェーズ影響**: Phase 1

#### P2-2: Stripe publishable key (pk_live) のHTMLハードコード
- **対象**: `aiden-order-checkout.html` line 761
- **内容**: `const STRIPE_PK = 'pk_live_51TAi...'` — publishable keyは公開前提だが、環境分離なし
- **修正方針**: 環境変数化
- **フェーズ影響**: Phase 1

#### P2-3: Edge Function呼び出しにanon keyをBearer使用
- **対象**: `aiden-order-checkout.html`, `aiden-customer-admin.html`, `aiden-order-dashboard.html`の複数箇所
