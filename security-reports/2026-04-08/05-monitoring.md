...
| 補償操作 | YES | weir-admin.html:1932 | writeAuditLog function |
| 管理エンティティCRUD | YES | weir-admin.html:245 | logAudit function |
...
- **対象**: `weir-admin.html` line 246
- **内容**: `user_email:'taisei@sumbibi.com'`がハードコード。全管理操作が同一メールで記録される。
- **修正方針**: `getCurrentUserEmail()`で動的取得（weir-customer-admin.htmlでは既に実装済み）
- **フェーズ影響**: Phase 1
...
- **対象**: weir-customer-admin.htmlのみunhandledrejectionハンドラーあり
- **フェーズ影響**: Phase 2
